...
5. **默认隔离开发。** 从最新 `origin/main` 或 `origin/master` 创建任务分支，并在独立 Git worktree 中开发。
...
中等和复杂任务的最终方案必须附一张紧凑的 **Mermaid 或 ASCII 示意图**，展示目标入口、关键模块或数据流及预期结果。多模块或流程方案优先使用 Mermaid，简单关系或渲染不确定时使用 ASCII；批准后将方案图保留在 Plan 文档中。该图描述拟议设计，任务完成时仍需另画一张反映实际改动的完成图。
...
方案批准后、持久化 Spec/Plan 和编码前，默认先获取 `origin` 最新状态，从实际存在的最新 `origin/main` 或 `origin/master` 创建新的任务分支和独立 `git worktree`；不得复用或改动用户当前工作区。仅当仓库没有 Git/对应远端分支、环境不支持 worktree 或用户明确要求时例外，并简要说明原因。
...
3. **命名：** 固定使用 `docs/specs/YYYY-MM-DD-<task-slug>.md` 和 `docs/plans/YYYY-MM-DD-<task-slug>.md`。日期取首次持久化时的本地日期；两份文档使用相同日期与小写 kebab-case task slug，后续更新不得改名。
...
任务完成后，最终汇报必须附一张清晰易懂的 **Mermaid 或 ASCII 示意图**，聚焦展示改动入口、主要改动点及其关系或影响。多模块或流程改动优先使用 Mermaid，简单改动或渲染不确定时使用紧凑 ASCII；不要绘制与本次任务无关的完整系统架构。
